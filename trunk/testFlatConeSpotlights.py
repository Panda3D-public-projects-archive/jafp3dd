...
class World(ShowBase):
    ...
    def __init__(self):
        ...
        taskMgr.add(self.moveLights,'Light Mover')
    ...
    def setupModels(self):
        ...
        self.model = loader.loadModel(os.path.join(_DATAPATH_,'models','plane'))
#        self.model = loader.loadModel(os.path.join(_DATAPATH_,'models','flatcone.wrl'))
        water = loader.loadTexture(os.path.join(_DATAPATH_,'textures','oceanTex2.png'))
        self.model.reparentTo(render)
        self.model.setTexture(water)
        self.model.setP(90)
        self.model.setScale(2000)
        self.model.setTransparency(1)
        self.model.setTwoSided(1)
        self.model.setColor((.9,.7,1,.5))
        cmat = Material()
        cmat.setShininess(2)
        cmat.setAmbient((1,2,1,1))
        cmat.setDiffuse((1,1,1,1))
        self.model.setMaterial(cmat)
...
#        camera.lookAt(self.sun.model)
    ...
    def moveLights(self,task):
        self.slnp.setZ( 50*(1+sin(task.time*2*pi/10)) )
        return task.cont
